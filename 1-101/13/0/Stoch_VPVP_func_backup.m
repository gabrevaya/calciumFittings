...
%         ncomp = 15;
        ncomp = 10;
        components = [1 3 4 5 8 9 14 15 17 20];
...
%         endd = 101;
        ...
        ncomp = 5;
        X_true = CCPP(beginn:endd-1,components);
        A = mean(std(X_true));
        X_true = X_true*(1/A);
        B = mean(X_true);
        X_true = bsxfun(@minus, X_true, B);
        x_start = X_true(beginn,components)';
...
%         if 0
        if or(mod(iter,optim_freq)==0, iter == optim_start),
            ...
            if iter == optim_start  
                VPVP_params.MaxIter = MaxIter_1st;
            else
